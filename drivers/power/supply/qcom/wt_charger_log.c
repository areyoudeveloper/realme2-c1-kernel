	...
	for (j = 0; j < len; j++) {
		...
		if (fg_chg_log_buf_offset + MAX_PROP_LEN < MAX_LOG_LEN) {
			...
			if (props[j] == POWER_SUPPLY_PROP_STATUS)
				fg_chg_log_buf_offset += snprintf(fg_chg_log_buf + fg_chg_log_buf_offset, MAX_PROP_LEN, "%s,", status_text[value.intval]);
			else if (props[j] == POWER_SUPPLY_PROP_REAL_STATUS)
				fg_chg_log_buf_offset += snprintf(fg_chg_log_buf + fg_chg_log_buf_offset, MAX_PROP_LEN, "%s,", status_text[value.intval]);
			else if (props[j] == POWER_SUPPLY_PROP_CHARGE_TYPE)
				fg_chg_log_buf_offset += snprintf(fg_chg_log_buf + fg_chg_log_buf_offset, MAX_PROP_LEN, "%s,", charge_type[value.intval]);
			else if (props[j] == POWER_SUPPLY_PROP_HEALTH)
				fg_chg_log_buf_offset += snprintf(fg_chg_log_buf + fg_chg_log_buf_offset, MAX_PROP_LEN, "%s,", health_text[value.intval]);
			else if (props[j] == POWER_SUPPLY_PROP_TECHNOLOGY)
				fg_chg_log_buf_offset += snprintf(fg_chg_log_buf + fg_chg_log_buf_offset, MAX_PROP_LEN, "%s,", technology_text[value.intval]);
			else if (props[j] == POWER_SUPPLY_PROP_CAPACITY_LEVEL)
				fg_chg_log_buf_offset += snprintf(fg_chg_log_buf + fg_chg_log_buf_offset, MAX_PROP_LEN, "%s,", capacity_level_text[value.intval]);
			else if (props[j] == POWER_SUPPLY_PROP_TYPE ||
					props[j] == POWER_SUPPLY_PROP_REAL_TYPE)
				fg_chg_log_buf_offset += snprintf(fg_chg_log_buf + fg_chg_log_buf_offset, MAX_PROP_LEN, "%s,", type_text[value.intval]);
			else if (props[j] == POWER_SUPPLY_PROP_SCOPE)
				fg_chg_log_buf_offset += snprintf(fg_chg_log_buf + fg_chg_log_buf_offset, MAX_PROP_LEN, "%s,", scope_text[value.intval]);
			else if (props[j] == POWER_SUPPLY_PROP_TYPEC_MODE)
				fg_chg_log_buf_offset += snprintf(fg_chg_log_buf + fg_chg_log_buf_offset, MAX_PROP_LEN, "%s,", typec_text[value.intval]);
			else if (props[j] == POWER_SUPPLY_PROP_TYPEC_POWER_ROLE)
				fg_chg_log_buf_offset += snprintf(fg_chg_log_buf + fg_chg_log_buf_offset, MAX_PROP_LEN, "%s,", typec_pr_text[value.intval]);
...
static struct class_attribute log_output_attributes[] = {
	__ATTR_RO(log),
	__ATTR_RO(log_head),
	__ATTR_NULL,
};
...
int init_fg_chg_work(struct smb_charger *chg)
{
	...
	g_smbchg_chg->log_output_class.class_attrs = log_output_attributes;
