...
#include <linux/pm_wakeup.h>
#define DTS_INT_GPIO	"novatek,irq-gpio"
#define DTS_RESET_GPIO	"novatek,reset-gpio"
...
static irqreturn_t ilitek_platform_irq_handler(int irq, void *dev_id)
{
	...
	if (core_gesture->suspend) {
		...
		pm_wakeup_event(&ipd->input_device->dev, 5000);
	}
...
static int ilitek_platform_isr_register(void)
{
	...
	ipio_debug(DEBUG_IRQ, "ipd->isr_gpio = %d\n", ipd->isr_gpio);
...
mutex_init(&ipd->ilitek_delta_mutex);
	...
	device_init_wakeup(&ipd->input_device->dev, 1);
	/*
