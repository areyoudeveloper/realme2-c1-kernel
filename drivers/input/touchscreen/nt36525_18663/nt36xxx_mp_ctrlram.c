...
int32_t nvt_selftest_open(struct inode *inode, struct file *file)
{
	...
	if(of_property_read_bool(np, "novatek,mp-support-dt")) {
		...
		if ( ctpmodule == 0 ){
		    ...
		    snprintf(mpcriteria, PAGE_SIZE, "novatek-mp-criteria-truly");
...
int oppo_nvt_blackscreen_test(void)
{
	...
	if ((TestResult_FW_Rawdata == 0) || (TestResult_FW_Rawdata == 1)) {
		if (TestResult_FWMutual == 0) {
			printk("FW Rawdata Test pass\n");
		}
	} else { // TestResult_FW_Rawdata is -1
		printk(" FW Rawdata FAIL!\n");
	}

	printk("Noise Test\n");
	if ((TestResult_Noise == 0) || (TestResult_Noise == 1)) {
		if (TestResult_FW_DiffMax == 0) {
			printk("Noise Test pass\n");
		} 
	} else { // TestResult_Noise is -1
		printk("Noise Test fail\n");
	}
	...
	if ((TestResult_FWMutual == 0)
		...
		sprintf(ts->gesture_test.message, "result=0");
	}
...
#endif
#if LPGW_TEST
	NVT_proc_lpwg_selftest_entry = proc_create("nvt_lpwg_selftest", 0664, ctp_device_proc, &nvt_lpwg_selftest_fops);
#endif
