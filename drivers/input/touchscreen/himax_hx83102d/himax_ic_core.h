...
struct himax_core_fp {
#ifdef CORE_IC
	int (*fp_burst_enable)(uint8_t auto_add_4_byte);
...
#ifdef HX_TP_baseline_test_DRIVER
	...
	void (*update_fw_func)(struct work_struct *work);
	int (*fp_0f_esd_check)(void);
	void (*fp_resume_work)(struct work_struct *work);
#ifdef HX_0F_DEBUG
