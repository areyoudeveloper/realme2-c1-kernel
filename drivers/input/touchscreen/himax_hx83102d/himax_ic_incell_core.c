...
extern int hx_ctpmodule;
	...
	extern char *i_CTPM_firmware_name_TCL;
	extern char *oppo_CTPM_firmware_name;
	extern char *oppo_CTPM_firmware_name_TCL;
	extern char *sort_fw_name_tcl;
	extern char *sort_fw_name;
	unsigned char fw_i_arr[]= {
	#include "himax_firmware_tcl.i"
	};
	extern u8 fw_arr[HX64K];
	extern u8 fw_arr_sort[HX64K];
	
	extern bool fw_arr_load_ok;
	extern bool fw_arr_sort_load_ok; 
#endif
...
static bool himax_mcu_sense_off(bool check_en)
{
	...
	g_core_fp.fp_register_write(pfw_op->addr_ctrl_fw_isr,
		sizeof(pfw_op->data_fw_stop), pfw_op->data_fw_stop, false);
...
static int himax_mcu_reload_disable(int disable)
{
	I("%s:entering\n", __func__);
...
static void himax_mcu_read_FW_ver(void)
{
	...
	while (reload_status == 0) {
		...
		if ((data[1] == 0x3A && data[0] == 0xA3)
			...
			msleep(10);
			if (retry % 10 == 0){
				I("reload fail ,delay 10ms retry=%d, reload_status=%d\n", retry, reload_status);
	...
	I("PANEL_VER : %X, FW_VER : %X, TOUCH_VER : %X, DISPLAY_VER : %X, CID_VER : %X\n",
    ...
    if ( hx_ctpmodule == 0 ) {
	    ...
	    devinfo_info_tp_set(version, "HLT",oppo_CTPM_firmware_name);
    } else {
        sprintf(Ctp_name,"HuaXian,HX83112a,FW:0x%x\n",ic_data->vendor_touch_cfg_ver);
        sprintf(version,"Tcl_Hx8311_0x%x",ic_data->vendor_touch_cfg_ver);
	    devinfo_info_tp_set(version, "TCL",oppo_CTPM_firmware_name_TCL);
	}
#endif
}
...
static void himax_mcu_touch_information(void)
{
    ...
    if ( hx_ctpmodule == 0 ) {
		...
		ic_data->HX_RX_NUM			= FIX_HX_RX_NUM_TCL;
	}
	...
	if ( hx_ctpmodule == 0 ) {
		...
		ic_data->HX_INT_IS_EDGE			= FIX_HX_INT_IS_EDGE_TCL;
	}
#endif
	...
	I("%s:RX_num =%d, TX_num =%d, MAX_PT=%d, XY_REV =%d, Y_RES =%d, X_RES=%d, INT_is_edge =%d\n"
	, __func__, ic_data->HX_RX_NUM, ic_data->HX_TX_NUM, ic_data->HX_MAX_PT
	, ic_data->HX_XY_REVERSE, ic_data->HX_Y_RES, ic_data->HX_X_RES, ic_data->HX_INT_IS_EDGE);
}
...
int hx_dis_rload_0f(int disable)
{
  ...
  do {
      ...
      g_core_fp.fp_register_read(pzf_op->addr_dis_flash_reload, 4, tmp_data, false);

      if( tmp_data[3] != 0x00 || tmp_data[2] != 0x00 || tmp_data[1] != 0x9A || tmp_data[0] != 0xA9) {
				I("Not Same,Write Fail, retry = %d, Now data: %02X%02X%02X%02X\n", retry,tmp_data[3], tmp_data[2], tmp_data[1], tmp_data[0]);
      } else {
        check_val = 0;
        I("It's same! Write success!\n");
      }
      msleep(5);
...
void himax_mcu_write_sram_0f(const struct firmware *fw_entry, uint8_t *addr, int start_index, uint32_t write_len)
{
	...
	uint8_t *tmp_data;
	uint32_t now_addr;
	unsigned char* fw_buff = fw_i_arr;
	int fw_size = sizeof(fw_i_arr);

	//I("%s, ---Entering \n", __func__);
	if (!private_ts->recovery_mode)
		I("%s, Entering normal mode\n", __func__);
	else
		I("%s, Entering recovery mode\n", __func__);
	if (fw_entry != NULL) {
		total_size = (!private_ts->recovery_mode)? fw_entry->size : fw_size;
		I("%s, FW entry is not NULL\n", __func__);
		//total_size = fw_entry->size;
	} else {
		total_size = fw_size;
		I("%s, FW entry is NULL\n", __func__);
	}
	...
	for (i = 0; i < (total_read_times); i++) {
		...
		if (total_size_temp >= max_bus_size) {
			...
			if (!private_ts->recovery_mode) {
				if (fw_entry != NULL) {
					memcpy (tmp_data, &fw_entry->data[start_index+i * max_bus_size], max_bus_size);
				} else if (fw_arr_load_ok == true && private_ts->in_baseline_test == 0) {
					memcpy (tmp_data, &fw_arr[start_index+i * max_bus_size], max_bus_size);
				} else if (fw_arr_sort_load_ok == true && private_ts->in_baseline_test == 1) {
					memcpy (tmp_data, &fw_arr_sort[start_index+i * max_bus_size], max_bus_size);
				}	else {
					memcpy (tmp_data, &fw_buff[start_index+i * max_bus_size], max_bus_size);
				}
			...
			if (!private_ts->recovery_mode) {
				if (fw_entry != NULL) {
					...
					memcpy (tmp_data, &fw_arr[start_index+i * max_bus_size], total_size_temp % max_bus_size);
				} else if (fw_arr_sort_load_ok == true && private_ts->in_baseline_test == 1) {
					memcpy (tmp_data, &fw_arr_sort[start_index+i * max_bus_size], total_size_temp % max_bus_size);
				}	else {
					memcpy (tmp_data, &fw_buff[start_index+i * max_bus_size], total_size_temp % max_bus_size);
				}
			}
			else
				memcpy (tmp_data, &fw_buff[start_index+i * max_bus_size], total_size_temp % max_bus_size);
...
int himax_sram_write_crc_check(const struct firmware *fw_entry, uint8_t *addr, int strt_idx, uint32_t len)
{
	...
	do {
		...
		crc = g_core_fp.fp_check_CRC (addr,  len);
			if (len == HX_48K_SZ && crc == 0) {
				temp_addr[3] = 0x08;
				temp_addr[2] = 0x00;
				temp_addr[1] = 0xBF;
				temp_addr[0] = 0xFC;
				g_core_fp.fp_register_read(temp_addr, 4, temp_data, false);
				I("%s, 48K LAST 4 BYTES: %02X%02X%02X%02X\n", __func__, temp_data[3], temp_data[2], temp_data[1], temp_data[0]);
				
				if (temp_data[3] == 0 && temp_data[2] == 0 && temp_data[1] == 0 && temp_data[0] == 0)
					crc = -1;
			}
...
int himax_mcu_0f_operation_dirly(void)
{
    ...
    I("%s, Entering \n", __func__);
     ...
     if (!private_ts->recovery_mode) {
        if(fw_arr_sort_load_ok == false)
        {
			I("===========reload firmware from file\n");
            if ( hx_ctpmodule == 0 ) {
				I("file name = %s\n", sort_fw_name);
				err = request_firmware (&fw_entry_temp, sort_fw_name, private_ts->dev);
			} else {
				I("file name = %s\n", sort_fw_name_tcl);
				err = request_firmware (&fw_entry_temp, sort_fw_name_tcl, private_ts->dev);
			}
		
			if (err < 0) {
				E("%s, fail error code=%d,file maybe fail need to load sort.bin\n", __func__, err);
				fw_arr_sort_load_ok = false;
				err = 0;//sorting firmware load fail without skip normal firmware load
				//return ;
			} else {
				memcpy(fw_arr_sort, fw_entry_temp->data, HX64K * sizeof(u8));
				I("FW local array is loaded, last 4 bytes of 48K = %02X, %02X, %02X, %02X\n",
				fw_arr_sort[HX_48K_SZ-4], fw_arr_sort[HX_48K_SZ-3], fw_arr_sort[HX_48K_SZ-2], fw_arr_sort[HX_48K_SZ-1]);
				release_firmware (fw_entry_temp);
				fw_arr_sort_load_ok = true;
			}
        }
        else
        {
            I("===========Update by local array\n");
        }

         if(fw_arr_load_ok == false)
        {
			I("===========reload firmware from file\n");
            if ( hx_ctpmodule == 0 ) {
				I("file name = %s\n", i_CTPM_firmware_name);
				err = request_firmware (&fw_entry_temp, i_CTPM_firmware_name, private_ts->dev);
			} else {
				I("file name = %s\n", i_CTPM_firmware_name_TCL);
				err = request_firmware (&fw_entry_temp, i_CTPM_firmware_name_TCL, private_ts->dev);
			}

			if (err < 0) {
				E("%s, fail error code=%d,file maybe fail need to load common.bin\n", __func__, err);
						fw_arr_load_ok = false;
				err = 0;//sorting firmware load fail without skip normal firmware load
				//return ;
			} else {
				memcpy(fw_arr, fw_entry_temp->data, HX64K * sizeof(u8));
				I("FW local array is loaded, last 4 bytes of 48K = %02X, %02X, %02X, %02X\n",
				fw_arr[HX_48K_SZ-4], fw_arr[HX_48K_SZ-3], fw_arr[HX_48K_SZ-2], fw_arr[HX_48K_SZ-1]);
				release_firmware (fw_entry_temp);
				fw_arr_load_ok = true;
			}
        }
	...
	if (!private_ts->recovery_mode) {
		if ( hx_ctpmodule == 0 ) {
			...
			I("Update by local array\n");
			//I("file name = %s\n", i_CTPM_firmware_name_TCL);
			//err = request_firmware (&fw_entry, i_CTPM_firmware_name_TCL, private_ts->dev);
		}

		/*if (err < 0) {
			E("%s, fail in line%d error code=%d,file maybe fail,need to load fw form .h\n", __func__, __LINE__, err);
			err = NO_ERR;
			//return err;
		}*/
...
void resume_fw_update_func(struct work_struct *work)
{
	int result = 0;
#ifdef HX_ZERO_FLASH	
	I("%s, enter \n", __func__);
	result = himax_mcu_0f_operation_dirly();
	if(result) {
...
void himax_mcu_0f_operation(struct work_struct *work)
{
	...
	int retry =0;
	const struct firmware *fw_entry = NULL;
	const struct firmware *fw_entry_sort = NULL;
        extern bool fw_arr_load_ok;
        extern bool fw_arr_sort_load_ok; 
	...
	if (!private_ts->recovery_mode) {
		do{
			if ( hx_ctpmodule == 0 ) {
				I("file name = %s\n", sort_fw_name);
				err = request_firmware (&fw_entry_sort, sort_fw_name, private_ts->dev);
			} else {
				I("file name = %s\n", sort_fw_name_tcl);
				err = request_firmware (&fw_entry_sort, sort_fw_name_tcl, private_ts->dev);
			}
			...
			if (err < 0) {
				...
				memcpy(fw_arr_sort, fw_entry_sort->data, HX64K * sizeof(u8));
				I("FW local array is loaded, last 4 bytes of 48K = %02X, %02X, %02X, %02X\n",
				fw_arr_sort[HX_48K_SZ-4], fw_arr_sort[HX_48K_SZ-3], fw_arr_sort[HX_48K_SZ-2], fw_arr_sort[HX_48K_SZ-1]);
				release_firmware (fw_entry_sort);
				fw_arr_sort_load_ok = true;
				break;
			}	
		}while(retry++<2);
		...
		do{
			...
			if (err < 0) {
				...
				memcpy(fw_arr, fw_entry->data, HX64K * sizeof(u8));
				I("FW local array is loaded, last 4 bytes of 48K = %02X, %02X, %02X, %02X\n",
				fw_arr[HX_48K_SZ-4], fw_arr[HX_48K_SZ-3], fw_arr[HX_48K_SZ-2], fw_arr[HX_48K_SZ-1]);
				fw_arr_load_ok = true;
				break;
	...
	if (!private_ts->recovery_mode && err >=0)
		release_firmware (fw_entry);
...
void himax_chip_resume_work(struct work_struct *work)
{
	int ret=0;
	int	retry_rst=3;
	I("%s: enter \n", __func__);
...
if ( !(private_ts->SMWP_enable)) {
	...
	if (private_ts->pdata->powerOff3V3 && private_ts->pdata->power) {
		private_ts->pdata->power(1);
	}
	...
	if(himax_mcu_0f_operation_dirly()) {
		E("Something is wrong! Skip Update with zero flash! %s: END \n",__func__);
		mutex_unlock(&private_ts->w_fw_lock);
		return;
	}
