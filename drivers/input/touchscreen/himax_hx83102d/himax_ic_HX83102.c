...
static bool hx83112_sense_off(bool check_en)
{
    ...
    if (cnt == 0 || (tmp_data[0] != 0xA5 && tmp_data[0] != 0x00 && tmp_data[0] != 0x87)) {
    	g_core_fp.fp_flash_write_burst(pfw_op->addr_ctrl_fw_isr, pfw_op->data_fw_stop);
    ...
    I("%s: save mode lock cnt = %d, data[0] = %2X!\n", __func__, cnt, tmp_data[0]);
  	} while (tmp_data[0] != 0x87 && (++cnt < 50) && check_en);
...
static void himax_hx83102d_reg_re_init(void)
{
	...
	//himax_in_parse_assign_cmd(hx83102d_fw_addr_fw_edge_limit, pfw_op->addr_fw_edge_limit, sizeof(pfw_op->addr_fw_edge_limit));
	//himax_in_parse_assign_cmd(hx83102d_fw_addr_fw_no_jiter_en, pfw_op->addr_fw_no_jiter_en, sizeof(pfw_op->addr_fw_no_jiter_en));
#if defined(HX_ZERO_FLASH)
...
static bool hx83102_chip_detect(void)
{
	...
	msleep(50);
	himax_pin_reset();/*tp reset*/
	hx83102_sense_off(false);
	printk("hx83102_chip_detect1111\n");
	...
	for (i = 0; i < 5; i++) {
		...
		if ((tmp_data[3] == 0x83) && (tmp_data[2] == 0x10) && ((tmp_data[1] == 0x2a) || (tmp_data[1] == 0x2b) || (tmp_data[1] == 0x2d))) {
			...
			ret_data = true;
			break;
		} else if ((tmp_data[3] == 0x83) && (tmp_data[2] == 0x11) && ((tmp_data[1] == 0x2a) || (tmp_data[1] == 0x2b))) {
			if (tmp_data[1] == 0x2a) {
