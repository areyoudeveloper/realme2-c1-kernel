...
const struct firmware *hx_entry_oppo = NULL;
								...
								size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
 		...
 		ret_data = g_core_fp.fp_register_read(pfw_op->addr_fw_ver_addr, FOUR_BYTE_DATA_SZ, tmp_data, 0);
		if (0 == ret_data)
			lenth += snprintf(temp_buf + lenth, len - lenth, "%s\n","i2c_device_test Pass!");
		else
			lenth += snprintf(temp_buf + lenth, len - lenth, "%s\n","i2c_device_test Fail!");
...
static struct file_operations himax_proc_i2c_device_test_ops = {
	.owner = THIS_MODULE,
	.read = himax_i2c_device_test_read,
};
#endif
...
void himax_log_touch_data(int start)
{
	...
	if (hx_touch_data->diag_cmd == 0) {
		...
		buf = kzalloc(/*hx_touch_data->touch_info_size*/print_size*sizeof(uint8_t), GFP_KERNEL);
		memcpy(buf, hx_touch_data->hx_coord_buf, hx_touch_data->touch_info_size);
...
/*
	do {
		ret = g_core_fp.fp_get_DSRAM_data(info_data, DSRAM_Flag);
		retry_cnt--;
	} while (ret == false && retry_cnt > 0);
*/
	index = 0;
	...
	for (i = 0; i < y_channel; i++) { /*mutual data*/
		for (j = 0; j < x_channel; j++) {
			...
			if (dsram_type == 1 || dsram_type == 4 || dsram_type == 5) {
								  ...
								  size_t len, loff_t *pos)
{
	size_t ret = 0;
	char *temp_buf;
	struct irq_desc *desc = irq_to_desc(private_ts->pdata->gpio_irq);
	I("%s: enter, %d \n", __func__, __LINE__);
...
/*
static ssize_t himax_coor_read(struct file *file, char *buf,
									size_t len, loff_t *pos)
{
	size_t ret = 0;
	char *temp_buf;
...
static void himax_oppo_raw_data_read(int ts_diag_cmd)
{
	...
	g_core_fp.fp_diag_register_set(diag_cmd, 0);
	...
	himax_int_enable(0);
retry:
	g_core_fp.fp_diag_register_set(diag_cmd, 1);
	msleep(20);

	/* 2. Start DSRAM thread */
	I("%s: Start get raw data in DSRAM\n", __func__);
	if (himax_ts_diag_func() == false && retry_cnt > 0) {
		retry_cnt--;
		goto retry;
	}
...
static ssize_t himax_oppo_reg_read_read(struct file *file, char *buf,/*oppo_register_info*/
										size_t len, loff_t *pos)
{
	int ret = 0;
	uint16_t loop_i;
...
#endif
static ssize_t himax_tp_fw_update_write(struct file *file, const char *buff, /*tp_fw_update*/
									size_t len, loff_t *pos)
{
	char buf_tmp[12] = {0};
	int res = 0;
	...
	else if(buf_tmp[0] != '0' && buf_tmp[0] != '1') {
		res = request_firmware_select(&hx_entry_oppo, i_CTPM_firmware_name_TCL,private_ts->dev);
		if (res) {
			E("update oppo  error\n");
		}
...
int himax_touch_proc_init(void)
{
	...
	if (himax_proc_i2c_device_test_file == NULL) {
		E(" %s: proc i2c device test file create failed!\n", __func__);
		goto fail_19;
	}
#endif
	himax_proc_irq_depth_file = proc_create(HIMAX_PROC_IRQ_DEPTH_FILE, 0666,
	...
	himax_proc_dbg_info_dir = proc_mkdir(HIMAX_PROC_DEBUG_INFO_FILE, himax_touch_proc_dir);

	if (himax_proc_dbg_info_dir != NULL) {
		himax_proc_delta_file = proc_create(HIMAX_PROC_DELTA_FILE, 0666,
									  himax_proc_dbg_info_dir, &himax_proc_delta_ops);
		if (himax_proc_delta_file == NULL) {
			E(" %s: proc dbg_info file create failed!\n", __func__);
			goto fail_oppo_3_1;
		}

		himax_proc_baseline_file = proc_create(HIMAX_PROC_BASELINE_FILE, 0666,
									  himax_proc_dbg_info_dir, &himax_proc_baseline_ops);
		if (himax_proc_baseline_file == NULL) {
			E(" %s: proc baseline file create failed!\n", __func__);
			goto fail_oppo_3_2;
		}

		himax_proc_main_reg_file = proc_create(HIMAX_PROC_MAIN_REG_FILE, 0666,
									  himax_proc_dbg_info_dir, &himax_proc_main_reg_ops);
		if (himax_proc_main_reg_file == NULL) {
			E(" %s: proc main_register file create failed!\n", __func__);
			goto fail_oppo_3_3;
		}
	} else {
		E(" %s: himax_proc_dbg_info_dir create failed!\n", __func__);
	}

	himax_proc_oppo_reg_info_file = proc_create(HIMAX_PROC_OPPO_REG_INFO_FILE, 0666,
								  himax_touch_proc_dir, &himax_proc_oppo_reg_read_ops);
	if (himax_proc_oppo_reg_info_file == NULL) {
		E(" %s: proc oppo_reg_info file create failed!\n", __func__);
		goto fail_oppo_7;
	}

	himax_proc_tp_fw_update_file = proc_create(HIMAX_PROC_TP_FW_UPDATE_FILE, 0666,
...
fail_oppo_8:
	remove_proc_entry(HIMAX_PROC_OPPO_REG_INFO_FILE, himax_touch_proc_dir);
fail_oppo_7:
	remove_proc_entry(HIMAX_PROC_MAIN_REG_FILE, himax_proc_dbg_info_dir);
fail_oppo_3_3:
	remove_proc_entry(HIMAX_PROC_BASELINE_FILE, himax_proc_dbg_info_dir);
fail_oppo_3_2:
	remove_proc_entry(HIMAX_PROC_DELTA_FILE, himax_proc_dbg_info_dir);
fail_oppo_3_1:
	remove_proc_entry(HIMAX_PROC_IRQ_DEPTH_FILE, himax_touch_proc_dir);
fail_oppo_1:
	remove_proc_entry(HIMAX_PROC_I2C_DEVICE_TEST_FILE, himax_touch_proc_dir);
fail_19:
